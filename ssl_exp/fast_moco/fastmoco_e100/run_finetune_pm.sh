...
k=config_forget_10_online
PORT=10105
echo "seleceted $PORT"
PYTHONPATH=$PYTHONPATH:../../../ \
srun --mpi=pmi2 --job-name=fastmoco -n8 -c 10 --gres=gpu:8 --ntasks-per-node=8  \
python -u -m core.solver.linear_solver --config linear_configs/$k.yaml 2>&1|tee logs/linear_$k-2.log &
